...
`v0.42.0`（2026-08-03 發布）的官方 release notes 顯示已修好造成 `v0.41.0` 被放棄的那兩個效能問題，是未來升級驗證基準的候選版本。2026-08-04 已對本機實際運行的 `v0.42.0` 完成部分實機驗證（`identify`／`capabilities`／`--workspace`／`--pane` 旗標、`WMUX_SURFACE_ID` env 存在與繼承，詳見 `wmux-best-practice/SKILL.md` 的「版本重新驗證紀錄（v0.42.0）」），但兩個技能檔案記錄的其餘具體行為結論（授權四類操作、`ok: true` 可靠性、跨 pane 交接、per-harness 適配層等）**尚未在 v0.42.0 上逐項重新驗證**，因此整體驗證基準仍維持 `v0.38.0` 不變，只有上述已列出的項目升級為已對 `v0.42.0` 驗證。
